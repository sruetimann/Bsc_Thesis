...
Establishes a per-glacier statistical relationship between MODIS NIR values and Landsat/Sentinel-2 SLA using Weighted Least Squares (WLS) regression. The fitted model is then applied to the full MODIS time series to generate a continuous 25-year SLA proxy, exported to `data/MODIS_data/predicted_sla/`.
...
Interpolates the composite SLA time series to a consistent daily resolution and computes annual and monthly mean SLA values for each glacier. Linear regression is applied to detect long-term trends (m/yr) and intra-seasonal variability across the study period.
...
Generates all publication-quality figures used in the thesis and presentation, including regression plots, multi-glacier comparisons, and seasonal SLA time series.
